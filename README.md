...
访问 [Haworthia OMICS Streamlit Demo](https://haworthia-omics-yujuncc.streamlit.app/) 可直接试用：
...
Demo 仅用于学术研究、教育和个人学习。预训练权重和数值原型只在托管运行时加载，不在
GitHub 仓库或 Release 中提供下载。请只上传您有权处理的图片；应用不会将上传图片加入训练集或
项目数据库，但托管平台可能在计算期间产生短期临时缓存。模型很可能出错，鉴定结果、注意力图、
表型网络、近邻关系和聚类均不得作为科研或实际决策的决定性依据。

## 最终模型的内部评测与重要限制

最终模型采用 300 轮训练、分类先验 `α=0.05`、质量感知降权和每类群 3 个子原型，训练图库
包含 126 个类群记录、3371 张人工筛选图片。此前在同一图库上进行的配对五折中心评估结果为：

- 类群/变种级 Top-1：`80.42%`；
- 物种级 Top-1：`99.85%`；
- 在新旧模型共同覆盖的 116 个类群上，按当前 3 子原型推理规则计算的类群级 Top-1：`87.29%`。

这些数值是**图库内评估**，不是独立外部测试准确率。网络权重已经见过这些图库图片；评估时
虽然将被测图片排除在对应折的中心之外，仍不能排除训练集记忆、图片来源风格和类群不平衡等
影响。项目尚无足够的、具有可靠标签且与训练图库完全独立的外部测试集，也没有建立经校准的
开放集未知类群检出率、误拒率或置信区间。因此，这些数字不能直接代表用户上传图片上的真实表现。

模型很可能出错，尤其是在背景分割失败、图片较暗或模糊、植株不完整、幼苗/胁迫状态、栽培状态
变化、冷门类群样本不足、类群间表型高度相近或输入类群不在原型库中时。开放集拒绝阈值也不等于
统计学置信度。鉴定名称、相似度排序、注意力热力图、表型网络、近邻表和聚类树均只能作为探索性
线索，**不得作为植物鉴定、分类学修订、亲缘/杂交判断、保护与交易决策或科研结论的决定性依据**。
重要结论应结合模式与原始描述、产地和生态信息、多个器官性状、可靠专家复核，以及在适用时的
分子系统学或群体遗传学证据。
